...
n_files_1 = [1440, 900, 1440, 1440, 1440, 1440, 1440, 1440, 780, 1440, 1440, 1440, 1440, 1440, 1983, 1463, 1440, 1440, 1440, 1440, 1121]
...
def get_livetime_window(sphere, dataset, data_prefix, nfile):
    ...
    # timestamps_file = np.empty(nfile)
    # window_count = np.empty(nfile)
    ...
    # Return up to the windows that pass livetime selection
    return good_window_times[:window_count]

def get_hist_per_dataset(sphere, datasets, bins, sidereal=True):
    print(sphere, sidereal)

    prefix_hist = 'sidereal' if sidereal else 'solar'

    out_dir = rf'E:\livetime_window\{sphere}'

    for dataset in datasets:
        outfile = rf'{sphere}_{dataset}_hist_livetime_window_{prefix_hist}.npz'
        ...
        # and store the rest as tmp
        if _idx_day.size == 2:
            _hist_0, _ = np.histogram(hours[ : _idx_day[0]+1], bins=bins)
            _hist_1, _ = np.histogram(hours[_idx_day[0]+1 : _idx_day[1]+1], bins=bins)
            _hist_2, _ = np.histogram(hours[_idx_day[1]+1 : ], bins=bins)
        elif _idx_day.size == 1:
            idx_day = _idx_day[0]
            _hist_0, _ = np.histogram(hours[:idx_day+1], bins=bins)
            _hist_1, _ = np.histogram(hours[idx_day+1:], bins=bins)
            _hist_2 = None
        else:
            _hist_0, _ = np.histogram(hours, bins=bins)
            _hist_1 = None
            _hist_2 = None

        print(f'Writing file {os.path.join(out_dir, outfile)}')
        np.savez(os.path.join(out_dir, outfile), hist_0=_hist_0, hist_1=_hist_1, hist_2=_hist_2)

if __name__ == "__main__":
    sphere_idx    = int(sys.argv[1])
    ...
    sidereal = True if time_type == 'sidereal' else False

    bins = np.linspace(0, 24, 25)
    get_hist_per_dataset(sphere, datasets, bins, sidereal=sidereal)
